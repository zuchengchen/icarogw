...
__all__ = [
    "initialize_prior_dict",
    "initialize_events_dict",
    "launch_analysis",
    "write_condor_files",
    "initialize_injections",
]
...
def initialize_prior_dict2(population, cosmology):
    ...
    if population == "BBH-mass_powerlaw-z_powerlaw":
    ...
    elif population in ["BBH-mass_powerlaw_gaussian-z_madau", "v_BBH-mass_powerlaw_gaussian-z_madau"]:
        prior_dict["alpha"] = _bilby.core.prior.Uniform(1.5, 12, name="alpha")
        prior_dict["beta"] = _bilby.core.prior.Uniform(-4, 12, name="beta")
        prior_dict["mmin"] = _bilby.core.prior.Uniform(2, 10, name="mmin")
        prior_dict["mmax"] = _bilby.core.prior.Uniform(50, 200, name="mmax")
        ...
        prior_dict["R0_abh"] = _bilby.core.prior.Uniform(0, 200, name="$R_{abh}$")

        if population == "v_BBH-mass_powerlaw_gaussian-z_madau":
            prior_dict["log_v0"] = _bilby.core.prior.Uniform(-5, -2, name="$\log_{10} v_0$")

    elif population == "PBH-lognormal_BBH-mass_powerlaw_gaussian-z_madau":
        prior_dict["mc"] = _bilby.core.prior.Uniform(5.0, 50.0, name="$M_c$")
        prior_dict["σc"] = _bilby.core.prior.Uniform(0.1, 2, name="$\\sigma_c$")
        prior_dict["log_fpbh"] = _bilby.core.prior.Uniform(
            -5.0, 0.0, name="$\\log_{10}\mathrm{fpbh}$"
        )
        prior_dict["alpha"] = _bilby.core.prior.Uniform(1.5, 12, name="alpha")
        prior_dict["beta"] = _bilby.core.prior.Uniform(-4, 12, name="beta")
        prior_dict["mmin"] = _bilby.core.prior.Uniform(2, 10, name="mmin")
        ...
        prior_dict["α1"] = _bilby.core.prior.Uniform(0.0, 15.0, name="$\\alpha_1$")
        prior_dict["α2"] = _bilby.core.prior.Uniform(1.0, 3.0, name="$\\alpha_2$")
        prior_dict["log_fpbh"] = _bilby.core.prior.Uniform(
            -4.0, 0.0, name="$\\log_{10}\mathrm{fpbh}$"
        )

    elif population in ["DW"]:
        prior_dict["α0"] = _bilby.core.prior.Uniform(1.0, 10.0, name="$\\alpha_0$")
        prior_dict["m0"] = _bilby.core.prior.Uniform(1.0, 5.0, name="$m_0$")
        prior_dict["λχ0"] = _bilby.core.prior.Uniform(0.1, 1.0, name="$\\lambda\\chi_0$")
        prior_dict["Φ0"] = _bilby.core.prior.Uniform(5.2, 5.8, name="$\\Phi_0$")
        prior_dict["log_fpbh"] = _bilby.core.prior.Uniform(
            -5.0, 0.0, name="$\\log_{10}\mathrm{fpbh}$"
        )
    ...
    if cosmology == "fixed-flatLCDM":
        ...
        prior_dict["Om0"] = _bilby.core.prior.DeltaFunction(0.315, name="$\Omega_m$")

    elif cosmology == "flatLCDM":
        prior_dict["H0"] = _bilby.core.prior.Uniform(10, 200, name="$H_0$")
        prior_dict["Om0"] = _bilby.core.prior.Uniform(0.0, 1.0, name="$\Omega_m$")
        ...
        prior_dict["H0"] = _bilby.core.prior.Uniform(10, 200, name="$H_0$")
        prior_dict["Om0"] = _bilby.core.prior.Uniform(0.1, 0.5, name="$\Omega_m$")
        prior_dict["w0"] = _bilby.core.prior.Uniform(-3.0, 0.0, name="$w_0$")
        prior_dict["wa"] = _bilby.core.prior.Uniform(-2.0, 2.0, name="$w_a$")

    elif cosmology == "restricted-flatLCDM":
        prior_dict["H0"] = _bilby.core.prior.Uniform(65.0, 77.0, name="$H_0$")
        prior_dict["Om0"] = _bilby.core.prior.DeltaFunction(0.315, name="$\Omega_m$")
    else:
...
def initialize_prior_dict(population, cosmology, rates, scale_free):
    ...
    if population == "BBH-powerlaw":
        ...
        prior_dict["alpha_1"] = _bilby.core.prior.Uniform(1.5, 12, name="alpha_1")
        prior_dict["alpha_2"] = _bilby.core.prior.Uniform(1.5, 12, name="alpha_2")
        prior_dict["beta"] = _bilby.core.prior.Uniform(-4, 12, name="beta")
        ...
        prior_dict["m_max"] = _bilby.core.prior.Uniform(60.0, 200.0, name="$m_{max}$")
...
def initialize_events_dict(
    runs, type=None, snr_cut=None, ifar_cut=None, to_include=None
):
    """
    ...
    for event in list(events_dict.keys()):
        ...
        if type is not None:
            if (
                (events_dict[event]["TYPE"] in type)
                and (events_dict[event]["SNR"] >= snr_cut)
                and (events_dict[event]["ifar"] >= ifar_cut)
            ):
                psd_events_out[event] = events_dict[event]
        else:
            if (events_dict[event]["SNR"] >= snr_cut) and (
                events_dict[event]["ifar"] >= ifar_cut
            ):
                psd_events_out[event] = events_dict[event]
