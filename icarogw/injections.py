...
from .utils.conversions import (
    source_frame_to_detector_frame as _source_frame_to_detector_frame,
)
from .utils.conversions import (
...
class injections_at_detector2:
    ...
    def __init__(self, injections):
        ...
        self.snr_cut = injections.snr_cut
        self.ntotal = injections.ntotal
        self.ntotal_original = injections.ntotal_original
        self.dl_original = injections.dl_original
        self.m1d_original = injections.m1d_original
        self.m2d_original = injections.m2d_original
        self.snr_original = injections.snr_original
        self.ini_prior_original = injections.ini_prior_original
        self.ifar = injections.ifar
        self.ifar_cut = injections.ifar_cut
        self.Tobs = injections.Tobs

        self.idet = injections.idet
        self.m1det = injections.m1det
        self.m2det = injections.m2det
        self.dldet = injections.dldet
        self.snrdet = injections.snrdet
        self.ini_prior = injections.ini_prior

    def cosmo_update(self, cosmo):
        self.ms1, self.ms2, self.z_samples = _detector_frame_to_source_frame(
            cosmo, self.m1det, self.m2det, self.dldet
        )

    def update_cut(self, snr_cut=0, ifar_cut=0, fraction=None):
        print(
            "Selecting injections with SNR {:f} and IFAR {:f} yr".format(
                snr_cut, ifar_cut
            )
        )
        ...
        idet = _np.where(
            (self.snr_original > self.snr_cut) & (self.ifar > self.ifar_cut)
        )[0]

        # Sub-sample the selected injections in order to reduce the computational load
        if fraction is not None:
            idet = _np.random.choice(
                idet, size=int(len(idet) / fraction), replace=False
            )
    ...
    def update_VT(self, population_prior, cosmo):

        if population_prior.name == "v_BBH-mass_powerlaw_gaussian-z_madau":
            v0 = 10 ** population_prior.hyper_params_dict["log_v0"]
            vs = 1 + get_z_doppler(v0, 1, len(self.ms1))[0]
            ms1 = self.ms1 / vs
            ms2 = self.ms2 / vs
        else:
            ms1 = self.ms1
        ...
        log_numer = population_prior.log_prob(
            ms1, ms2, self.z_samples)
        log_jacobian_term = _np.log(
            _np.abs(_detector_to_source_jacobian(
                self.z_samples, cosmo, dl=self.dldet))
        )
        self.log_weights_astro = log_numer - \
            _np.log(self.ini_prior) - log_jacobian_term
        ...
        self.VT_sens = _np.exp(_logsumexp(
            self.log_weights_astro)) / self.ntotal

    def expected_number_detection(self):
        """
        This method will return the expected number of GW detection given the injection set.
        Tobs is automatically saved in the class creation
...
class injections_at_detector:
    ...
    def __init__(
        ...
        idet = _np.where((self.snr_original > snr_cut) &
                         (self.ifar > self.ifar_cut))[0]
    ...
    def update_VT(self, m_prior, z_prior):
        ...
        self.ms1, self.ms2, self.z_samples = _detector_frame_to_source_frame(
            self.new_cosmo, self.m1det, self.m2det, self.dldet
        )
        ...
        if self.condition_check:
            ...
            if (_np.max(mass_a) < m_prior.mmax) | (_np.min(mass_a) > m_prior.mmin):
                print(
                    "The  injections source frame masses are not enough to cover all the prior range"
                )
                print(
                    "Masses prior range {:.2f}-{:.2f} Msol".format(
                        m_prior.mmin, m_prior.mmax
                    )
                )
                print(
                    "Injections range {:.2f}-{:.2f} Msol".format(
                        _np.min(mass_a), _np.max(mass_a)
                    )
                )
                _sys.exit()

        # Calculate the weights according to Eq. 18 in the document
        log_numer = z_prior.log_prob_astro(self.z_samples) + m_prior.log_joint_prob(
            self.ms1, self.ms2
        )
        log_jacobian_term = _np.log(
            _np.abs(
                _detector_to_source_jacobian(
                    self.z_samples, self.new_cosmo, dl=self.dldet
                )
            )
        )
        self.log_weights_astro = log_numer - \
    ...
    def return_reweighted_injections(self, new_samples=5000):
        ...
        new_index = _np.random.choice(
            len(self.z_samples), size=new_samples, p=weights, replace=True
        )
        return (
            self.ms1[new_index],
            self.ms2[new_index],
            self.z_samples[new_index],
            total_weight,
            new_index,
        )
    ...
    def calculate_Neff(self):
        ...
        var = (
            _np.exp(_logsumexp(self.log_weights * 2)) / (self.ntotal**2)
            - (mean**2) / self.ntotal
        )
        return (mean**2) / var
    ...
    def simulate_moc(
        self, n_moc=100, Nsamp=5000, mode_likelihood="delta", filepath="."
    ):
        """
        ...
        ms1, ms2, zz, _, index = self.return_reweighted_injections(
            new_samples=n_moc)
        md1, md2, dl = _source_frame_to_detector_frame(
            self.new_cosmo, ms1, ms2, zz)

        eta = md1 * md2 / _np.power(md1 + md2, 2)
        chirp_mass = (md1 + md2) * _np.power(eta, 3.0 / 5)
        ...
        for i in range(n_moc):
            ...
            print(
                "The injection has m1d {:.2f} Msol, m2d {:.2f} Msol, dl {:.0f} Mpc and SNR {:.1f}".format(
                    md1[i], md2[i], dl[i], det_snr[i]
                )
            )

            if mode_likelihood == "uniform":
                d_uni = _np.random.uniform(low=0, high=dlmax, size=Nsamp * 500)
                md1_uni = _np.random.uniform(
                    low=0, high=md1max, size=Nsamp * 500)
                md2_uni = _np.random.uniform(
                    low=0, high=md1max, size=Nsamp * 500)
                ...
                sub_index = _np.random.choice(
                    len(d_uni), size=Nsamp, p=weights, replace=True
                )
            ...
            _np.savez(
                filepath + "injection_{:d}.moc".format(i),
                dl=d_uni[sub_index],
                md1=md1_uni[sub_index],
                md2=md2_uni[sub_index],
                dl_true=dl[i],
                md1_true=md1[i],
                md2_true=md2[i],
                snr_det=det_snr[i],
...
class injections_at_source:
    ...
    def __init__(
        ...
        md1, md2, dl = _source_frame_to_detector_frame(
            self.cosmo_ref, self.m1s_original, self.m2s_original, self.z_original
        )
        idet = _np.where((self.snr_original > snr_cut) &
                         (self.ifar > self.ifar_cut))[0]
        ...
        self.log_origin_jacobian = _np.log(
            _np.abs(
                _detector_to_source_jacobian(
                    self.z_original[self.idet], self.cosmo_ref, dl=self.dldet
                )
            )
        )
