...
class injections_at_detector2():
    ...
    def update_cut(self, snr_cut=0, ifar_cut=0, fraction=None):
        print('Selecting injections with SNR {:f} and IFAR {:f} yr'.format(
            snr_cut, ifar_cut))

        self.snr_cut = snr_cut
        self.ifar_cut = ifar_cut
        idet = _np.where((self.snr_original > self.snr_cut) &
                         (self.ifar > self.ifar_cut))[0]

        # Sub-sample the selected injections in order to reduce the computational load
        if fraction is not None:
            idet = _np.random.choice(idet, size=int(
                len(idet)/fraction), replace=False)
    ...
    def update_VT(self, population_prior, cosmo):
        ...
        log_jacobian_term = _np.log(_np.abs(_detector_to_source_jacobian(
...
class injections_at_detector():
    ...
    def update_VT(self, m_prior, z_prior):
        ...
        if self.condition_check:
            ...
            if (_np.max(mass_a) < m_prior.mmax) | (_np.min(mass_a) > m_prior.mmin):
                ...
                print(
                    'Masses prior range {:.2f}-{:.2f} Msol'.format(m_prior.mmin, m_prior.mmax))
                print(
                    'Injections range {:.2f}-{:.2f} Msol'.format(_np.min(mass_a), _np.max(mass_a)))
                _sys.exit()

        # Calculate the weights according to Eq. 18 in the document
        log_numer = z_prior.log_prob_astro(
            self.z_samples) + m_prior.log_joint_prob(self.ms1, self.ms2)
        log_jacobian_term = _np.log(_np.abs(_detector_to_source_jacobian(
    ...
    def calculate_Neff(self):
        ...
        var = _np.exp(_logsumexp(self.log_weights*2)) / \
            (self.ntotal**2) - (mean**2)/self.ntotal
        return (mean**2)/var
    ...
    def simulate_moc(self, n_moc=100, Nsamp=5000, mode_likelihood='delta', filepath='.'):
        ...
        for i in range(n_moc):
            ...
            print('The injection has m1d {:.2f} Msol, m2d {:.2f} Msol, dl {:.0f} Mpc and SNR {:.1f}'.format(
...
class injections_at_source():
    ...
    def __init__(self, cosmo_ref, m1s, m2s, z, prior_vals, snr_det, snr_cut, ifar, ifar_cut, ntotal, Tobs, condition_check=False):
        ...
        self.log_origin_jacobian = _np.log(_np.abs(_detector_to_source_jacobian(
    ...
    def update_VT(self, m_prior, z_prior):
        ...
        log_numer = z_prior.log_prob_astro(
            self.z_samples)+m_prior.log_joint_prob(self.ms1, self.ms2)
        log_jacobian_term = _np.log(_np.abs(_detector_to_source_jacobian(
            self.z_samples, self.new_cosmo, dl=self.dldet)))-self.log_origin_jacobian
        self.log_weights_astro = log_numer - \
            _np.log(self.ini_prior)-log_jacobian_term
        self.log_weights = self.log_weights_astro - _np.log(z_prior.norm_fact)
        # This is the Volume-Time in which we expect to detect. You can multiply it by R_0 Tobs to get the expected number of detections in Gpc^3 yr
