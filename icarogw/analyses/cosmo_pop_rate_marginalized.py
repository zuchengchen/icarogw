...
class BBH_likelihood2(_bilby.Likelihood):
    ...
    def __init__(self, posterior_samples_dict, injections, population_model, cosmo_model, parallel=2000, fixed_cosmo=False):
        ...
        # Save the total list of parameters
        tot_list = self.list_population_param + self.list_cosmo_param
    ...
    def log_likelihood(self):
        ...
        if self.parallel:
            ...
            # Calulcate the poissonian term of Eq. 7 if the prior on rate is not log uiniform
            N_exp = self.injections.expected_number_detection()
            N_obs = len(self.posterior_samples_dict)
            T_obs = self.injections.Tobs
            log_poissonian_term = N_obs * _np.log(T_obs) - N_exp

            # Combine all the terms
            log_likeli = log_poissonian_term + log_numerator
...
class hierarchical_analysis():
    ...
    pos_samples_dict: dic
        dictionary containing poster samples classes of the events you want to analyse
...
class hierarchical_analysis2():
    ...
    def __init__(self, pos_samples_dict, injections, fixed_cosmo=False):
        self.pos_samples_dict = pos_samples_dict
        self.injections = injections
        self.fixed_cosmo = fixed_cosmo
