...
class posterior_samples(object):
    ...
    def __init__(self, filename=None, waveform=None, mass_1_det=None, mass_2_det=None, distance=None, size=None):
        """
        ...
        if filename is None:
            ...
            self._load_posterior_samples(size=size)
    ...
    def _load_posterior_samples(self, size=None):
        """
        ...
        if (size!=None) and (size<self.nsamples):
            idx = _np.random.choice(range(0, self.nsamples), size=size)
            self.mass_1_det_original = _copy.deepcopy(self.mass_1_det)
            self.mass_2_det_original = _copy.deepcopy(self.mass_2_det)
            self.distance_original = _copy.deepcopy(self.distance)
            
            self.nsamples = size
            self.mass_1_det = self.mass_1_det_original[idx]
            self.mass_2_det = self.mass_2_det_original[idx]
            self.distance = self.distance_original[idx]
            
        print(f"Using {waveform} posterior with a total of {self.nsamples} samples")
