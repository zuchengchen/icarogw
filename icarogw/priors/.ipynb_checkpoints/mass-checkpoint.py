...
class mass_prior(object):
    ...
    def log_joint_prob(self, ms1, ms2):
        ...
        if self.name == 'PBH-lognormal':
            to_ret = log_R_pbh_log(ms1, ms2, self.mc, self.σc, self.m_min, self.m_max) - log_R_pbh_log_norm(self.mc, self.σc, self.m_min, self.m_max)
        elif self.name == 'PBH-lognormal-nocut':
            to_ret = log_R_pbh_log_nocut(ms1, ms2, self.mc, self.σc) - log_R_pbh_log_nocut_norm(self.mc, self.σc)  
            
        elif self.name == 'PBH-powerlaw':
            to_ret = log_R_pbh_power(ms1, ms2, self.α, self.m_min, self.m_max) - log_R_pbh_power_norm(self.α, self.m_min, self.m_max)
        elif self.name == 'PBH-powerlaw-nocut':
            to_ret = log_R_pbh_power_nocut(ms1, ms2, self.α, self.m_min) - log_R_pbh_power_nocut_norm(self.α, self.m_min)
            
        elif self.name == 'PBH-cc':
            to_ret = log_R_pbh_cc(ms1, ms2, self.α, self.Mf, self.m_min, self.m_max) - log_R_pbh_cc_norm(self.α, self.Mf, self.m_min, self.m_max)
        elif self.name == 'PBH-cc-nocut':
            to_ret = log_R_pbh_cc_nocut(ms1, ms2, self.α, self.Mf) - log_R_pbh_cc_nocut_norm(self.α, self.Mf)
        else:
            # Returns the joint probability Factorized as in Eq. 33 on the paper
