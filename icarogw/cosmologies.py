...
class flatLCDM(object):
    ...
    def __init__(self, Omega_m=0.308, H0=67, zmax=10, astropy_conv=False):
        ...
        log_dl_trials = _np.log10(
            self.astropy_cosmology.luminosity_distance(z_array).value)
        log_dvc_dz_trials = _np.log10(
            4 * _np.pi * self.astropy_cosmology.differential_comoving_volume(z_array).to(_u.Gpc**3/_u.sr).value)
        log_dt_trials = _np.log10(self.astropy_cosmology.age(z_array).value)
        ...
        self.interp_z_to_dt = _splrep(log_z_array, log_dt_trials, s=0)
    ...
    def z_at_dl(self, dl):
        ...
        if self.astropy_conv:
            ...
            z_ret = _np.nan_to_num(
                10.**_splev(_np.log10(dl), self.interp_dl_to_z, ext=0))
        return z_ret
    ...
    def t_at_z(self, z):
        """
        Returns cosmic time given cosmological parameters
        ...
        return _np.nan_to_num(10.**_splev(_np.log10(z), self.interp_z_to_dt, ext=0))
